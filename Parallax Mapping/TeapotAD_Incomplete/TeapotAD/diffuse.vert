...
void main()
{
   vertPos = vec3(M*V*vec4(VertexPosition,1.0)); 
     
   lightPos = vec3( M*V*vec4(LightPosition,1.0));  

   N = normalize( NormalMatrix * VertexNormal);

   lightDirection = normalize(SpotPosition - VertexPosition);
   targetDirection = normalize(targetDirection - SpotPosition);
   pos_eye = vec3(MVP*vec4(VertexPosition,1.0));
   ...
   uvCoord = TextureCoords; 
	...
	vec3 vector = light - vertPos;
	TangentSurface2Light = mat * vector;

	vector = pos_eye - vertPos;
